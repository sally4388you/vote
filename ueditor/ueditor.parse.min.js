...
function(a,c){c=c||function(a,c){return a.localeCompare(c)};for(var b=0,d=a.length;b<d;b++)for(var f=b,g=a.length;f<g;f++)if(0<c(a[b],a[f])){var h=a[b];a[b]=a[f];a[f]=h}return a}}});UE.parse.register("charts",function(f){function h(a){var c=[];a=a.getElementsByTagName("table");for(var b=0,d;d=a[b];b++)null!==d.getAttribute("data-chart")&&c.push(g(d));return c.length?c:null}function g(a){for(var c=a.getAttribute("data-chart"),b={},d=[],f=0,e;e=a.rows[f];f++){for(var g=[],k=0,h;h=e.cells[k];k++){var m=
h.innerText||h.textContent||"";g.push("TH"==h.tagName?m:m|0)}d.push(g)}c=c.split(";");for(f=0;e=c[f];f++)e=e.split(":"),b[e[0]]=e[1];return{table:a,meta:b,data:d}}function a(){window.jQuery?b():f.loadFile(document,{src:c+"/third-party/jquery-1.10.2.min.js",tag:"script",type:"text/javascript",defer:"defer"},function(){b()})}function b(){window.Highcharts?d():f.loadFile(document,{src:c+"/third-party/highcharts/highcharts.js",tag:"script",type:"text/javascript",defer:"defer"},function(){d()})}function d(){f.loadFile(document,
{src:c+"/dialogs/charts/chart.config.js",tag:"script",type:"text/javascript",defer:"defer"},function(){for(var a=null,c=null,b=null,d=0,e=k.length;d<e;d++){var a=k[d],c=[],f=[],b=[],g=a.data,h=a.meta;if("1"!=h.dataFormat){for(var p=0,f=g.length;p<f;p++)for(var m=0,t=g[p].length;m<t;m++)b[m]||(b[m]=[]),b[m][p]=g[p][m];g=b}b={};if(h.chartType!=typeConfig.length-1){f=g[0].slice(1);p=1;for(m=void 0;m=g[p];p++)c.push({name:m[0],data:m.slice(1)});b.series=c;b.categories=f;b.title=h.title;b.subTitle=h.subTitle;
b.xTitle=h.xTitle;b.yTitle=h.yTitle}else{m=[];p=1;for(f=g[0].length;p<f;p++)m.push([g[0][p],g[1][p]|0]);c[0]={type:"pie",name:h.tip,data:m};b.series=c;b.title=h.title}b.suffix=h.suffix;c=b;b=a.table;g=document.createElement("div");g.className="edui-chart-container";b.parentNode.replaceChild(g,b);b=g;a=typeConfig[a.meta.chartType];$(b).highcharts($.extend({},a,{credits:{enabled:!1},exporting:{enabled:!1},title:{text:c.title,x:-20},subtitle:{text:c.subTitle,x:-20},xAxis:{title:{text:c.xTitle},categories:c.categories},
